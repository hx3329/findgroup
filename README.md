...
Since we’re building our app from scratch, we’ll use ES6 and JSX to write as little code as possible, as well as the
okta React SDK for user features.
...
- Challenger
- Okta – Allows us to store and authenticate users without having to
  create our own backend for it.
- Okta Auth SDK used by kta-signin-widget and Okta-auth-js
  Okta-signin-widget - built with specific flows like providing
  Okta-auth-js - change the flow in some way like making our own registerService.
